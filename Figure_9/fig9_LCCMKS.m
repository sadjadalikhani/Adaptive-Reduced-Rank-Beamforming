...
for trial = 1:trials 
    ...
    for i = 1:N
       ...
       %% APPROXIMATION OF AUTO-CORRELATION FUNCTION
        ...
        %% SINR OUT 
        wTilde = gamma*steerVec(:,desiredAngle)-B*Tr*w;
        SINRout(i,trial) = real((wTilde'*Rdesired*wTilde)/(wTilde'*Rinterference*wTilde));
    end
    clc
end
SINR = mean(pow2db(SINRout),2);

plot(1:N,SINR,'LineWidth',1,"DisplayName","LCCM-KS") 
